...
public class focusTest : MonoBehaviour
{
    ...
    {
        if (target.transform.position == _targetPos)
            return;
        transform.position = target.transform.position + _offset;
        // transform.RotateAround(target.transform.position, target.transform.up, Quaternion.Angle(transform.rotation, target.transform.rotation));
        transform.LookAt(target);
        _targetPos = target.transform.position;
    }
